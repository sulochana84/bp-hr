...
def video_clip(folder):
    ...
    ffmpeg_extract_subclip(folder, 5, 20, targetname="clipped_video/test.MOV")
...
def main(folder):
    ...
    video_namepd = pd.DataFrame(video_name)
    video_namepd.columns = ['Video_Name']
    video_namepd.to_csv('Patient_Video_name.csv', index=True)
    ...
    df = pd.DataFrame(arr, video_namepd, columns)
    ...
    sys_pressure = [ 90, 100,120,150, 95 ,110, 120,107,100,99]
    dys_pressure = [60,70,80,85,80,120,87,65,79,95]
    pickled_model  = bp_regression(sys_pressure, dys_pressure)
